...
//
//int count = 0, countL = 0, countT = 0, countQ = 0, draw_mode, Color_mode = Red; // 점의 개수, 그림형태, 색(초기 값 빨강);
//int x_point[10] = { 0 }, y_point[10] = { 0 }; //좌표 기억을 위한 배열
//int x_line[10] = { 0 }, y_line[10] = { 0 };
//int x_tri[10] = { 0 }, y_tri[10] = { 0 };
//int x_quad[10] = { 0 }, y_quad[10] = { 0 };
//
...
//			printf("%d %d\n", x, 500 - y);
//			x_line[countL] = x;
//			y_line[countL] = 500 - y; // 원래는 왼쪽 위가 좌표 0.0 이기에 아래쪽을 0.0으로 설정하면 마우스의 좌표받는 함수를 바꿔주어야 함.
//			countL++; // 점의 개수 증가
//		}
...
//			printf("%d %d\n", x, 500 - y);
//			x_tri[countT] = x;
//			y_tri[countT] = 500 - y; // 원래는 왼쪽 위가 좌표 0.0 이기에 아래쪽을 0.0으로 설정하면 마우스의 좌표받는 함수를 바꿔주어야 함.
//			countT++; // 점의 개수 증가
//		}
//		else if (draw_mode == quadrangle)
//		{
//			printf("%d %d\n", x, 500 - y);
//			x_quad[countQ] = x;
//			y_quad[countQ] = 500 - y; // 원래는 왼쪽 위가 좌표 0.0 이기에 아래쪽을 0.0으로 설정하면 마우스의 좌표받는 함수를 바꿔주어야 함.
//			countQ++; // 점의 개수 증가
//		}
//	}
